...
class ImgProc(object):
    ...
    def frame_sub(self, src, dst, roi, draw) -> list[float]:
        result: list[float] = []
         ...
         # self.prevが未設定なら初期化して何もせずreturn
        if not hasattr(self, "prev") or self.prev is None:
            self.prev = src.copy()
            result.append(0.0)
            return result
        ...
        # --- Sobelフィルタを適用 or スキップ ---
        if not roi.get("NoSobel", False):
            sobel = cv2.Sobel(sub, cv2.CV_64F, 1, 0, ksize=3)
